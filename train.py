...
if __name__ == '__main__':
    ...
    parser.add_argument('--signal_lambda', type=float, default=0.0) # signal loss
    ...
    signal_lambda = args.signal_lambda
    ...
    for trials in range(num_trials):
        ...
        for epoch in range(num_epoch):
            ...
            for i, X_batch in enumerate((trainloader), 0):
                ...
                loss4 = criterion(Output[:, 3], (X_batch[:, 3] - S0min) / (S0max - S0min))
                # signal_loss = criterion(Signal, Signal_pred)

                loss = lambda1*loss1 + lambda2*loss2 + lambda3*loss3 #+ signal_lambda*signal_loss
                loss.backward()
            ...
            with torch.no_grad():
                for i, X_batch_val in enumerate((validationloader), 0):
                    ...
                    loss4 = criterion(Output_val[:, 3], (X_batch_val[:, 3] - S0min) / (S0max - S0min))

                    # signal_loss = criterion(Signal / S0max, Signal_pred / S0max)
                    loss = lambda1*loss1 + lambda2*loss2 + lambda3*loss3 #+ signal_lambda*signal_loss
